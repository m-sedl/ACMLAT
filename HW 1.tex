...
Тогда можно выбрать только те наборы аргументов, 
которые функция отображает в 1. Для каждого такого набора аргументов 
\(a_1,...,a_n\)
...
\[M \llbracket \phi \rrbracket = MAX(M_1[...], ..., M_{2^n}[...]) = f(M(p_1), ..., M(p_n))\]
